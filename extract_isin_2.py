...
def main():
    ...
    # get all xetra symbols as a file
    repsonse_content = f.get_xetra_symbol_file()
    if repsonse_content:
        with open("./data/symbols_xetra.csv", "wb") as file:
                        file.write(repsonse_content)

     # currently disabled as file has changed. Use last working file
    ...
    df_xetra_check = df_xetra_check.reset_index(drop=True).iloc[0:2400].copy()       
    ...
    print("xetra check file shape after yfin: ", df_xetra_check.shape)
    ...
    print("check final file shape after links: ", df_check_final.shape)
